...
export function SkillCard({ skill, toolEntries, index = 0, onToggle, onOpen }: SkillCardProps) {
  ...
  // shooting-star hover: each card's orbit starts from its own angle so a
  // grid of hovered cards never sweeps in lockstep (stable per skill)
  const sweepOffset = useMemo(() => {
    let h = 0;
    for (const ch of skill.id) h = (h * 31 + ch.charCodeAt(0)) % 360;
    return `${h}deg`;
  }, [skill.id]);
  ...
  return (
    <motion.div
      ...
      style={{ "--sweep-offset": sweepOffset } as CSSProperties}
